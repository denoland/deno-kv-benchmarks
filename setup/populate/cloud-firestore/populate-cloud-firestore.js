...
try {
  ...
  gcpProjectId = gcpProjectIdOutput.type === "string"
    ? gcpProjectIdOutput.value
    : "";
  gcpFirestoreCollection = gcpFirestoreCollectionOutput.type === "string"
    ? gcpFirestoreCollectionOutput.value
    : "";
} catch (_error) {
...
if (!gcpProjectId) {
  console.error(
    "error: failed to get GCP Project ID from the Terraform .tfstate. Did you run Terraform?",
  );
  process.exit(1);
}

const datasetFilepath = readFileSync(
