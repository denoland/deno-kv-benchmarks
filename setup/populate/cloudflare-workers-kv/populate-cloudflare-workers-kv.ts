...
try {
  ...
  cfAccountId = cfAccountIdOutput.type === "string"
    ? cfAccountIdOutput.value
    : "";
  cfKvNamespaceId = cfKvNamespaceIdOutput.type === "string"
    ? cfKvNamespaceIdOutput.value
    : "";
} catch (_error) {
...
if (!cfAccountId) {
  console.error(
    "error: failed to get the Cloudflare Account ID from the Terraform .tfstate. Did you run Terraform?",
  );
  Deno.exit(1);
}
...
if (!cfKvNamespaceId) {
  console.error(
    "error: failed to get the Cloudflare KV Namespace ID from the Terraform .tfstate. Did you run Terraform?",
  );
  Deno.exit(1);
}
...
async function countKeys(
  ...
  while (cursor) {
    const url =
      `https://api.cloudflare.com/client/v4/accounts/${accountId}/storage/kv/namespaces/${namespace}/keys?limit=1000&prefix=${
