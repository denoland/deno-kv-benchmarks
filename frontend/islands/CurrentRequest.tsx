...
function Wrapper(
  { children, ready = false }: { children: ComponentChildren; ready?: boolean },
) {
  const bgClass = ready ? "bg-[#E8570C]" : "bg-gray-400";
  const verticalPadding = ready ? "pt-8 pb-10 md:py-5" : "py-7";
  return (
    <div
      class={`w-full flex flex-row flex-wrap lg:flex-nowrap ${verticalPadding} md:py-0 items-center justify-center ${bgClass}`}
    >
      {children}
    </div>
  );
...
export default function CurrentRequest() {
  ...
  const latencyItems = Object.entries(
    latencyDataResponse.currentRequestLatencies,
  );
      ...
      <div class="md:flex hidden lg:min-w-0 min-w-[33.333%] sm:flex-grow lg:w-full lg:h-full flex-col items-center justify-center py-3 xl:py-0">
        <div class="text-left md:text-center">
          ...
          <div class="text-base lg:text-sm">
            <span>
              Read{" "}
              <span class="font-bold">{Math.floor(data.latencies.read)}ms</span>
            </span>
            ...
            <span>
              ...
              <span class="font-bold">
                {Math.floor(data.latencies.write)}ms
              </span>
            </span>
          </div>
        </div>
  ...
  const latencyTableRows = latencyItems
    .map(([service, data]) => (
      <tr class="border-b-1 border-white border-opacity-10 last:border-b-0">
        ...
        <td class="py-1 text-right pr-5">
          <span class="font-bold">{Math.floor(data.latencies.write)}ms</span>
        </td>
      </tr>
    ));
    ...
    <>
      <Wrapper ready={true}>
        ...
        <table class="table md:hidden w-full">
          <thead>
            <tr class="font-bold">
              <td class="py-2 border-b-1 border-white border-opacity-25 pl-5">
                Service
              </td>
              <td class="py-2 border-b-1 border-white border-opacity-25 text-right">
                Read Latency
              </td>
              <td class="py-2 border-b-1 border-white border-opacity-25 text-right pr-5">
                Write Latency
              </td>
            </tr>
          </thead>
